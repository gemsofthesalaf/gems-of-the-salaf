...
CREATE TABLE public.quotes (
    ...
    search_vector_ar tsvector GENERATED ALWAYS AS (to_tsvector('simple', coalesce(arabic_text, ''))) STORED,
    
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    published_at TIMESTAMPTZ,
    CONSTRAINT quotes_published_arabic_required
      CHECK (status <> 'published' OR nullif(trim(arabic_text), '') IS NOT NULL)
);
...
CREATE INDEX quotes_translator_id_idx ON public.quotes(translator_id);
CREATE INDEX quotes_status_published_idx ON public.quotes(status, published_at DESC, id);
CREATE INDEX quotes_featured_published_idx ON public.quotes(featured, published_at DESC) WHERE status = 'published';
...
CREATE INDEX quote_categories_category_idx ON public.quote_categories(category_id, quote_id);
CREATE INDEX quote_tags_tag_idx ON public.quote_tags(tag_id, quote_id);
CREATE UNIQUE INDEX admins_email_lower_idx ON public.admins(lower(email));
...
CREATE TRIGGER tags_updated_at BEFORE UPDATE ON public.tags FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
...
-- Archived taxonomy records are not part of the public archive.
...
-- No direct authenticated-client write policies are installed. NextAuth CMS writes
-- execute only through the server-only service role after requireAdmin().
